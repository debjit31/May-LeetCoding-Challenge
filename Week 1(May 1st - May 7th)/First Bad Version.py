...
# will return whether version is bad. Implement a function to find the first bad version.
...
## isBadVerison() is a system built API to check for bad version

def firstBadVersion(version):
    ## Binary Search 
    left = 1
    right = n
    while (left < right):
